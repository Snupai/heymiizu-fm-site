      ...
      <nav className="w-full bg-white shadow-md flex flex-col items-center justify-between px-4 py-3" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 3000 }}>
        ...
        {menuOpen && (
          <div className="w-full bg-white border-t border-gray-200 mt-2 flex flex-col items-center animate-fade-in" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.07)' }}>
            ...
            {isAdmin && <Link href="/admin" className="block py-2 text-lg w-full text-center" onClick={() => setMenuOpen(false)}>Admin</Link>}
            {user ? (
              <button onClick={() => { void signOut(); setMenuOpen(false); }} className="block py-2 text-lg w-full text-center text-red-500">Logout</button>
            ) : (
              <Link href="/login" className="block py-2 text-sm w-full text-center text-gray-400" onClick={() => setMenuOpen(false)}>Login</Link>
            )}
          </div>
        )}
      </nav>
          ...
          <div className="flex items-center gap-8">
            <ul className="flex gap-8 text-lg items-center">
              <li className="inline-flex flex-col items-center">
                <motion.div initial="initial" whileHover="hover" animate="initial" className="relative group">
                  <Link href="/" className="relative block group" onClick={(e) => handleLinkClick(e, "/")}>
                    <span className={`relative z-10 transition-colors duration-200 ${pathname === "/" ? "text-brand" : ""}`}>Home
                      <motion.span className="block h-[2px] bg-brand absolute left-0 bottom-0" style={{ width: '100%' }} initial={{ scaleX: 0, originX: 0 }} variants={{ hover: { scaleX: 1, originX: 0 } }} transition={{ duration: 0.3 }} />
                    </span>
                  </Link>
                </motion.div>
              </li>
              <li className="inline-flex flex-col items-center">
                <motion.div initial="initial" whileHover="hover" animate="initial" className="relative group">
                  <Link href="/projects" className="relative block group" onClick={(e) => handleLinkClick(e, "/projects")}>
                    <span className={`relative z-10 transition-colors duration-200 ${pathname === "/projects" ? "text-brand" : ""}`}>Projects
                      <motion.span className="block h-[2px] bg-brand absolute left-0 bottom-0" style={{ width: '100%' }} initial={{ scaleX: 0, originX: 0 }} variants={{ hover: { scaleX: 1, originX: 0 } }} transition={{ duration: 0.3 }} />
                    </span>
                  </Link>
                </motion.div>
              ...
              <li className="inline-flex flex-col items-center">
                <motion.div initial="initial" whileHover="hover" animate="initial" className="relative group">
                  <Link href="/contact" className="relative block group" onClick={(e) => handleLinkClick(e, "/contact")}>
                    <span className={`relative z-10 transition-colors duration-200 ${pathname === "/contact" ? "text-brand" : ""}`}>Contact Me
                      <motion.span className="block h-[2px] bg-brand absolute left-0 bottom-0" style={{ width: '100%' }} initial={{ scaleX: 0, originX: 0 }} variants={{ hover: { scaleX: 1, originX: 0 } }} transition={{ duration: 0.3 }} />
                    </span>
                  </Link>
                </motion.div>
              ...
              {isAdmin && (
                <li className="inline-flex flex-col items-center">
                  <motion.div initial="initial" whileHover="hover" animate="initial" className="relative group">
                    <Link href="/admin" className="relative block group" onClick={(e) => handleLinkClick(e, "/admin")}>
                      <span className={`relative z-10 transition-colors duration-200 ${pathname === "/admin" ? "text-brand" : ""}`}>Admin
                        <motion.span className="block h-[2px] bg-brand absolute left-0 bottom-0" style={{ width: '100%' }} initial={{ scaleX: 0, originX: 0 }} variants={{ hover: { scaleX: 1, originX: 0 } }} transition={{ duration: 0.3 }} />
                      </span>
                    </Link>
                  </motion.div>
            ...
            <div className="flex items-center gap-4">
              {user ? (
                <button onClick={() => signOut()} className="text-sm font-medium text-gray-600 hover:text-black transition-colors">
                  Logout
                </button>
              ) : (
