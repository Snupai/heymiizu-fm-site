...
export const UsersList = ({ refreshTrigger }: UsersListProps) => {
    ...
    const updateRole = async (userId: string, newRole: "admin" | "pending" | "denied") => {
                ...
                <Card key={user.id}>
                    <CardContent className="p-4 flex items-center justify-between">
                        <div className="space-y-1">
                            ...
                            <p className="text-xs font-mono text-muted-foreground">ID: {user.user_id}</p>
                            <Badge variant={
                                user.role === "admin" ? "default" :
                                    user.role === "denied" ? "destructive" : "secondary"
                            }>
                                {user.role}
                            </Badge>
                        </div>
