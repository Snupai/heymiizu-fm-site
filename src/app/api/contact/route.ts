...
export async function PUT(req: NextRequest) {
  ...
  if (!body || typeof body !== 'object' || !('paused' in body) || typeof body.paused !== 'boolean') {
    return NextResponse.json({ error: 'A valid paused state is required.' }, { status: 400 });
  }

  const pauseUntil = 'pauseUntil' in body ? body.pauseUntil : null;
  if (pauseUntil !== null && (typeof pauseUntil !== 'string' || !isValidDateKey(pauseUntil))) {
    return NextResponse.json({ error: 'The pause end date is invalid.' }, { status: 400 });
  }

  if (body.paused && pauseUntil && pauseUntil < getDateKeyInTimeZone()) {
    return NextResponse.json({ error: 'The pause end date cannot be in the past.' }, { status: 400 });
  }
  ...
  if (error) {
    ...
    return NextResponse.json({ error: 'Could not save contact form settings.' }, { status: 500 });
  }
